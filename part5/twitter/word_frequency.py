...
def main():
    ...
    # コマンドラインの第一期日数で、WikiEctractorの出力先のディレクトリを指定する。
...
def count_words(file: TextIO) -> Counter:
    '''
    WikiEctractorが出力したファイルに含まれる全ての記事から単語の出現回数を数える回数
    '''
...
def iter_doc_countents(file: TextIO) -> Iterator[str]:
    '''
    ファイルオブジェクトを読み込んで、記事の中身(開始タグ<doc ...>と終了タグ</doc>の間のテキスト)を
    順に返すジェネレーター関数。
    '''
    for line in file: # ファイルに含まれる全ての行について反復処理する。
...
def get_words(content: str) -> List[str]:
    ...
    while node:
        ...
        # 最初の2項目をposとpos_sub1に体入する。posはPart of Speech(品詞)の略。
